function [C_normal_alpha_1] = aerosurface_calc(mach, incidence, aspect_ratio, midchord_angle)
    ...
    elseif mach > supersonic
        K_n = [((gamma+1)*mach^8+(2*gamma^2-7*gamma-5)*mach^6+10*(gamma+1)*mach^4+8)/(6*beta^7); 
             ((gamma+1)*mach^4-4*beta^2)/(4*beta^4);  
              2/beta];
        C_normal_alpha_1 = polyval(K_n, incidence); %Taken from waterloo, unverified
    else
        %Entirely taken from waterloo hopefully this works
